...
export default z
	.array(
		z
			.object({
				id: idPrimitive
					.describe('ID of the target receiver to apply parameters to'),
				params: z
					.object({
						sender_id:
							idPrimitive
						...
						activation: z
							.object({
								mode: z.union([z.literal('activate_immediate'), z.literal('activate_scheduled_absolute'), z.literal('activate_scheduled_relative')]).nullable()
									.describe(
								...
								requested_time: z.string().regex(new RegExp('^[0-9]+:[0-9]+$')).nullable()
									.describe(
	...
	.describe('Describes a bulk receiver update resource')
