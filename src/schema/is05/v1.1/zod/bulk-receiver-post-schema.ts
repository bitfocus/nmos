...
// Activation parameters schema
const activationSchema = z
	.object({
		mode: z
			.union([
				z.literal('activate_immediate'),
				z.literal('activate_scheduled_absolute'),
				z.literal('activate_scheduled_relative'),
			])
			.nullable()
			.describe(
...
const rtpTransportSchema = z
	.object({
		...
		destination_port: portOrAutoSchema
...
// Receiver parameters schema
const receiverParamsSchema = z
	.object({
...
// Individual receiver update item schema
const receiverUpdateItemSchema = z
	.object({
		id: idPrimitive.describe('ID of the target receiver to apply parameters to'),
		params: receiverParamsSchema,
	})
	.strict()

// Main bulk receiver update schema
export default z.array(receiverUpdateItemSchema).describe('Describes a bulk receiver update resource')
